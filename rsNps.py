...
# 5. 점수 구간 만들기
def make_bin(score):
    bin_start = (score - 1) // 5 * 5 + 1 if score > 0 else 0
    bin_end = bin_start + 4
    return f"{bin_start}-{bin_end}"

df_합산["점수구간"] = df_합산["점수"].apply(make_bin)

# 6. 구간별 학생 수 집계
구간_분포 = df_합산["점수구간"].value_counts().sort_index()
df_구간분포 = pd.DataFrame({
    "점수구간": 구간_분포.index,
    "학생수": 구간_분포.values
})
...
fig_bins = px.bar(
    ...
    title=f"{선택학년}학년 기준 최신 합산점수 구간별 학생 수"
)
